...
//#region node_modules/kareem/index.js
var require_kareem = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	Kareem.prototype.execPre = function(name, context, args, callback) {
		...
		if (!numPres) return nextTick(function() {
			callback(null);
		});
		function next() {
			if (currentPre >= numPres) return;
			const pre = pres[currentPre];
			if (pre.isAsync) {
				const args = [decorateNextFn(_next), decorateNextFn(function(error) {
					if (error) {
						if (done) return;
						if (error instanceof Kareem.skipWrappedFunction) shouldSkipWrappedFunction = error;
						else {
							done = true;
							return callback(error);
						}
					}
					if (--asyncPresLeft === 0 && currentPre >= numPres) return callback(shouldSkipWrappedFunction);
				})];
				...
				for (let i = 1; i < _args.length; ++i) {
					if (i === _args.length - 1 && typeof _args[i] === "function") continue;
					args.push(_args[i]);
				}
				callMiddlewareFunction(pre.fn, context, args, args[0]);
			} else {
				...
				if (isPromiseLike(maybePromiseLike)) maybePromiseLike.then(() => _next(), (err) => _next(err));
				else {
					if (++currentPre >= numPres) if (asyncPresLeft > 0) return;
					else return nextTick(function() {
		...
		function _next(error) {
			...
			if (++currentPre >= numPres) if (asyncPresLeft > 0) return;
			else return callback(shouldSkipWrappedFunction);
	...
	*/
	Kareem.prototype.execPreSync = function(name, context, args) {
		const pres = this._pres.get(name) || [];
		const numPres = pres.length;
		for (let i = 0; i < numPres; ++i) pres[i].fn.apply(context, args || []);
	};
	/**
	...
	*/
	Kareem.prototype.execPost = function(name, context, args, options, callback) {
		...
		if (!numPosts) return nextTick(function() {
		...
		function next() {
			...
			for (let i = 0; i < argLength; ++i) {
				...
				if (!args[i] || !args[i]._kareemIgnore) newArgs.push(args[i]);
			}
			if (firstError) if (isErrorHandlingMiddleware(posts[currentPost], numArgs)) {
				const _cb = decorateNextFn(function(error) {
					if (error) {
						if (error instanceof Kareem.overwriteResult) {
							args = error.args;
							if (++currentPost >= numPosts) return callback.call(null, firstError);
							return next();
						}
						firstError = error;
					}
				...
				const _cb = decorateNextFn(function(error) {
					if (error) {
						if (error instanceof Kareem.overwriteResult) {
							args = error.args;
							if (++currentPost >= numPosts) return callback.apply(null, [null].concat(args));
							return next();
						}
						firstError = error;
						return next();
				...
				if (post.length === numArgs + 1) callMiddlewareFunction(post, context, newArgs.concat([_cb]), _cb);
				else {
					let error;
					...
					if (isPromiseLike(maybePromiseLike)) return maybePromiseLike.then((res) => {
						_cb(res instanceof Kareem.overwriteResult ? res : null);
					}, (err) => _cb(err));
					if (maybePromiseLike instanceof Kareem.overwriteResult) args = maybePromiseLike.args;
					if (++currentPost >= numPosts) return callback.apply(null, [error].concat(args));
					next();
				}
			}
		}
	...
	*/
	Kareem.prototype.createWrapperSync = function(name, fn) {
		...
		return function syncWrapper() {
			...
			return _this.execPostSync(name, this, [toReturn])[0];
		};
	};
	function _handleWrapError(instance, error, name, context, args, options, callback) {
		if (options.useErrorHandlers) return instance.execPost(name, context, args, { error }, function(error) {
			return typeof callback === "function" && callback(error);
		});
		else return typeof callback === "function" && callback(error);
	}
	/**
	...
	Kareem.prototype.wrap = function(name, fn, context, args, options) {
		...
		this.execPre(name, context, args, function(error) {
			if (error && !(error instanceof Kareem.skipWrappedFunction)) {
				...
				for (let i = errorArgs.length; i < numCallbackParams; ++i) errorArgs.push(null);
			...
			if (checkForPromise) {
				if (isPromiseLike(ret)) return ret.then((res) => _cb(null, res), (err) => _cb(err));
				if (numParameters < argsWithoutCb.length + 1) return _cb(null, ret);
			}
			function _cb() {
				const argsWithoutError = Array.from(arguments);
				argsWithoutError.shift();
				if (options.nullResultByDefault && argsWithoutError.length === 0) argsWithoutError.push(null);
				if (arguments[0]) return _handleWrapError(_this, arguments[0], name, context, argsWithoutError, options, lastArg);
				else _this.execPost(name, context, argsWithoutError, function() {
	...
	*/
	Kareem.prototype.createWrapper = function(name, fn, context, options) {
		const _this = this;
		if (!this.hasHooks(name)) return function() {
	...
	*/
	Kareem.prototype.pre = function(name, isAsync, fn, error, unshift) {
		...
		if (unshift) pres.unshift(Object.assign({}, options, {
			fn,
			isAsync
		}));
		else pres.push(Object.assign({}, options, {
			fn,
			isAsync
		}));
		return this;
	...
	*/
	...
	function callMiddlewareFunction(fn, context, args, next) {
		...
		if (isPromiseLike(maybePromiseLike)) maybePromiseLike.then(() => next(), (err) => next(err));
	}
